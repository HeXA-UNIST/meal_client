...
Data/Infra        → features/announcement/data/ (announcement_api, announcement_service)
                    features/meal/data/ (meal_api, meal_data_source)
...
- **Announcement service**: `features/announcement/data/announcement_service.dart` is intentionally split from `announcement_api.dart` so that `checkAnnouncementString()` (SharedPreferences comparison) can be unit-tested independently of HTTP. `checkForNewAnnouncement()` composes both.
