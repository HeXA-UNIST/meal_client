...
- **Android home widgets**: Implemented natively under `android/app/src/main/kotlin/pro/hexa/meal/meal_client`. Widgets are cache-only: `BapUWidgetMealRepository` reads fresh `context.filesDir/meal.json`, `BapUWidgetOperatingHours` reads `context.filesDir/info.json`, and missing/stale/corrupt cache produces empty/unknown widget state without attempting native HTTP. `BapUWidgetUpdateDispatcher` renders all providers; `BapUWidgetScheduleManager` uses AlarmManager for display boundaries (midnight, 09:21, 13:31, operating start, closing-soon). The legacy `BapUWidgetUpdateWorker` only cancels old scheduled WorkManager work and does not render.
