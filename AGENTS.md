...
- Home screen widget native wiring is not on this branch. The `develop-widget` branch has a finished cache-only Android implementation (`android/app/.../BapUWidget*.kt`, `plugins/bapu_widget_bridge/`) that reads `meal.json`/`info.json` directly with no native network fallback; merge that in rather than re-deriving the older network-fallback design from `docs/shared-meal-cache-refactor-plan.md`.
