...
- **Language**: Hardcoded to Korean — locale detection is commented out and replaced with `true` in `main.dart` (lines 21, 39). Non-standard English support infrastructure exists but is disabled.
...
- i18n improvement (migrate to Flutter's official localization framework)
- Settings screen
...
- UI strings are wrapped in `MultiLanguageString` with `eng`/`kor` fields, accessed via `.getLocalizedString(language)`
