...
@objc class AppDelegate: FlutterAppDelegate, FlutterImplicitEngineDelegate {
  ...
  override func application(
    ...
    Self.registerBapUWidgetStorageChannel(with: self)
    WorkmanagerPlugin.setPluginRegistrantCallback { registry in
      GeneratedPluginRegistrant.register(with: registry)
      Self.registerBapUWidgetStorageChannel(with: registry)
    }
  ...
  func didInitializeImplicitFlutterEngine(_ engineBridge: FlutterImplicitEngineBridge) {
    ...
    Self.registerBapUWidgetStorageChannel(with: engineBridge.pluginRegistry)
  }

  private static func registerBapUWidgetStorageChannel(with registry: FlutterPluginRegistry) {
    guard let registrar = registry.registrar(forPlugin: "BapUWidgetStorageBridge") else {
      return
    }
    registerBapUWidgetStorageChannel(binaryMessenger: registrar.messenger())
  }

  private static func registerBapUWidgetStorageChannel(binaryMessenger: FlutterBinaryMessenger) {
